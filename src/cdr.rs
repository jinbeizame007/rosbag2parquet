...
impl<'a> CdrDeserializer<'a> {
    ...
    #[inline]
    pub fn next_bytes<'b>(&mut self, data: &'b [u8], count: usize) -> &'b [u8] {
        self.position += count;
    ...
    pub fn read_sequence_length(&mut self) -> u32 {
        self.align_to(4);
        let header = self.next_bytes(self.data, 4);
    ...
    pub fn deserialize_f64(&mut self) -> f64 {
        self.align_to(8);
        let bytes = self.next_bytes(self.data, 8);
        match self.byte_order {
    ...
    pub fn deserialize_f32(&mut self) -> f32 {
        self.align_to(4);
        let bytes = self.next_bytes(self.data, 4);
        match self.byte_order {
    ...
    pub fn deserialize_bool(&mut self) -> bool {
        let bytes = self.next_bytes(self.data, 1);
        bytes[0] == 0x01
    }
    ...
    pub fn deserialize_char(&mut self) -> char {
        let byte = self.next_bytes(self.data, 1)[0];
        byte as char
    }

    pub fn deserialize_i16(&mut self) -> i16 {
        self.align_to(2);
        let bytes = self.next_bytes(self.data, 2);
        match self.byte_order {
    ...
    pub fn deserialize_string(&mut self) -> String {
        ...
        let bytes = self.next_bytes(self.data, byte_length as usize);
